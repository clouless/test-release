sh 'curl -sSLko pipeline-helper.groovy ${K8S_INFRASTRUCTURE_BASE_URL}pipeline-helper/pipeline-helper.groovy?v8'
...
pipelineHelper.nodejsTemplate {
  ...
  stage('release') {
    dir ('source') {
      ...
      pipelineHelper.test123('x1')
      pipelineHelper.githubUploadAssetToGitHubRelease(pipelineHelper.githubUploadAssetToGitHubRelease()"clouless", env.GWBT_REPO_NAME, "1.0", releaseId, "dist", "my_asset.json", 'application/json')
    }
  }
}
